...
removeResults(function(nr, err) {
	sysLogger.info('<db_prices> <removeResults> Success!'); 
});  
...
function closeConnection() {
	mongoose.connection.close( function(err) {
		if(err) { 
			sysLogger.warn('<db_prices> <closeConnection> '+ 'No Mongoose Connection: Nothing to close...');
		}
	});
}
...
function executeQuery(callback) {
	getQuery(function(query,err) {
		...
		if(err) return sysLogger.error('<db_prices> <executeQuery> '+ err);
		 	sysLogger.info(docs);
		 	callback();
		});	
  	});
...
function getQuery(callback) {
	...
	mongoose.connect('mongodb://' + rtc.getConfig('logs.host') + ':' + rtc.getConfig('logs.port') + '/' + rtc.getConfig('logs.db'), function(err, db) {
		if(err) { sysLogger.error('<db_prices> <getQuery> '+ err); }
...
function removeResults(callback) {
	...
	mongoose.connect('mongodb://' + rtc.getConfig('logs.host') + ':' + rtc.getConfig('logs.port') + '/' + rtc.getConfig('logs.db'), function(err, db) {
		if(err) { sysLogger.error('<db_prices> <removeResults> '+ err); }
		...
		if(from != null && to != null) 
			...
			.remove(function(err, numberRemoved) {
				sysLogger.info('<db_prices> <removeResults> Remove logs with eventId = ' + eventId); 
				callback(numberRemoved);		
			});	
		else { 
			Model.find({})		
			.remove(function(err, numberRemoved) {
				sysLogger.info('<db_prices> <removeResults> Removed complete history, number of lines: ' + numberRemoved); 
				callback(numberRemoved);		
			});
		}
	});
...
function streamResults(res, callback) {
	sysLogger.info('<db_prices> <streamResults> Accessing logs at ' + 'mongodb://' + rtc.getConfig('logs.host') + ':' + rtc.getConfig('logs.port') + '/' + rtc.getConfig('logs.db'));
...
function testQuery() {
	console.log('<db_prices> <testQuery>');	
