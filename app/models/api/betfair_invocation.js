...
exports.startInvocationLog = function (streams) {
    ...
    streams.forEach(function (item) {
        ...
        if (filename && fs.existsSync(filename)) {
            sysLogger.info('<betfair_invocation> <startInvocationLog> filename = ' + filename + ' exists ? ' + fs.existsSync(filename));
            var now = new Date();
...
BetfairInvocation.prototype.execute = function (callback) {
    ...
    if (self.isEmulated && isEmulatedCall) {
        transport = emulator;
        sysLogger.info('<betfair_invocation> <execute> Emulated Call - Transport: ' + JSON.stringify(transport,null,2));
    }
