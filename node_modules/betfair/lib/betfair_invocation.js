...
exports.startInvocationLog = function (streams) {
    ...
    streams.forEach(function (item) {
        ...
        if (filename && fs.existsSync(filename)) {
            sysLogger.crit('<betfair_invocation> <startInvocationLog> filename = ' + filename + ' exists ? ' + fs.existsSync(filename));
            var now = new Date();
...
BetfairInvocation.prototype.execute = function (callback) {
    ...
    if (self.isEmulated && isEmulatedCall) {
        transport = emulator;
        sysLogger.crit('<betfair_invocation> <execute> Emulated Call - Transport: ' + JSON.stringify(transport,null,2));
    }
...
BetfairInvocation.prototype.end = function () {
	...
	sysLogger.crit('<betfair_invocation> <end> self.response = ' + JSON.stringify(self.response, null, 2));
	

    self.isSuccess = ((self.response && self.response.result) ? true : false);
    self.duration = self.finishDate - self.startDate;

	sysLogger.crit('<betfair_invocation> <end> self.isSuccess: ' + self.isSuccess);
