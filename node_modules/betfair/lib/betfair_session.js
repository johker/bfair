...
// listCurrentOrders
BetfairSession.prototype.listCurrentOrders = function (params, cb) {
    var self = this;

    cb = cb || function () {
    };
    var isEmulated = self.isMarketUsingBetEmulator(params.marketId);
    sysLogger.notice('<betfair_session> <listCurrentOrders> isEmulated = ' + isEmulated);
    var inv = new BetfairInvocation("sports", self.sessionKey, "listCurrentOrders", params, isEmulated);
    inv.execute(function (err, inv) {
        cb(err, inv);
    });
}
...
BetfairSession.prototype.placeOrders = function (params, cb) {
    ...
    var isEmulated = self.isMarketUsingBetEmulator(params.marketId);
	sysLogger.notice('<betfair_session> <placeOrders> isEmulated = ' + isEmulated);
